...
app.logger.debug('Modelo y métricas cargados correctamente.')

# Features que RFE seleccionó (guardadas en train_model.py)
FEATURES = metricas.get('features', ['reading score', 'writing score',
                                      'lunch', 'test preparation course'])

# Mappings para variables categóricas (LabelEncoder fit_transform orden alfabético)
ENCODINGS = {
...
# Opciones para mostrar en el formulario HTML
FORM_OPTIONS = {
    'gender':                    ['female', 'male'],
...
CATEGORICAL_FIELDS = list(ENCODINGS.keys())
...
def predict():
    try:
        ...
        for feat in FEATURES:
            raw = request.form.get(feat.replace(' ', '_'))
        ...
        app.logger.debug(f'Input: {data_df}')
